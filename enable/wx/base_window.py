...
class LessSuckyDropTarget(PythonDropTarget):
    ...
    def OnDragOver(self, x, y, default_drag_result):
        ...
        if hasattr(self.handler, 'wx_drag_over'):
            drag_result = self.handler.wx_drag_over(x, y, clipboard.data,
                                                    default_drag_result)
        else:
...
class BaseWindow(AbstractWindow):
        ...
        if event is not None:
            ...
            mouse_wheel = (
                (event.GetLinesPerAction() *
                 event.GetWheelRotation()) / (event.GetWheelDelta() or 1))
    ...
    def _get_control_size(self):
        ...
        result = None
        if self.control:
            result = self.control.GetSize().Get()
        return result
    ...
    def set_pointer(self, pointer):
        ...
        elif not (type(ptr) is wx.Cursor):
            POINTER_MAP[pointer] = ptr = wx.Cursor(1)  #arrow/default
