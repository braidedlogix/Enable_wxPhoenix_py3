...
class SVGDocument(object):
    ...
    def getLocalState(self, element, state=None):
        ...
        if state is None:
            state = self.state
        current = dict(state)
        element_items = [(k, v) for (k, v) in list(element.items())
                         if v != 'inherit']
        current.update(element_items)
        style_items = [
            (k, v)
            for (k, v
                 ) in list(css.inlineStyle(element.get("style", "")).items())
            if v != 'inherit'
        ]
    ...
    def addUseToDocument(self, node):
        ...
        try:
            ...
            warnings.warn("Could not find SVG file %s. %s: %s" %
    ...
    def addImageToDocument(self, node):
        ...
        try:
            ...
            warnings.warn("Could not find image file %s. %s: %s" %
    ...
    def addRectToDocument(self, node, path):
        ...
        if 'clip-path' in list(node.keys()):
    ...
    def getPenFromState(self):
        ...
        pen.SetCap(
            self.renderer.caps.get(
                self.state.get('stroke-linecap', None), self.renderer.caps[
                    'butt']))
        pen.SetJoin(
            self.renderer.joins.get(
                self.state.get('stroke-linejoin', None), self.renderer.joins[
                    'miter']))
    ...
    def parseStops(self, element):
        ...
        for stop in element:
            ...
            row = (offset, color[0] / 255., color[1] / 255., color[2] / 255.,
                   opacity)
            stops.append(row)
        stops.sort()
        if len(stops) == 0:
    ...
    def addStrokeToPath(self, path, stroke):
        ...
        if type == 'M':
            ...
            control1, control2, endpoint = list(map(normalizePoint, arg))
            ...
            control2, endpoint = list(map(normalizePoint, arg))
            if self.lastControl:
            ...
            path.elliptical_arc_to(rx, ry, angle, large_arc_flag, sweep_flag,
                                   x2, y2)
