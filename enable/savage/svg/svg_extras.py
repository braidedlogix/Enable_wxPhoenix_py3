...
def bezier_arc(x1, y1, x2, y2, start_angle=0, extent=90):
    ...
    for i in range(nfrag):
        ...
        if frag_angle > 0:
            ...
            signed_kappa = kappa
        point_list.append(
            (x_cen + rx * c0, y_cen - ry * s0, x_cen + rx *
             (c0 + signed_kappa * s0), y_cen - ry * (s0 - signed_kappa * c0),
             x_cen + rx * (c1 - signed_kappa * s1), y_cen - ry *
             (s1 + signed_kappa * c1), x_cen + rx * c1, y_cen - ry * s1))
...
def elliptical_arc_to(path, rx, ry, phi, large_arc_flag, sweep_flag, x1, y1,
    ...
    arcs = []
    control_points = bezier_arc(cxp - rx, cyp - ry, cxp + rx, cyp + ry, theta1,
                                dtheta)
    for x1p, y1p, x2p, y2p, x3p, y3p, x4p, y4p in control_points:
        # Transform them back to asbolute space.
        args = (transform_from_local(x2p, y2p, cphi, sphi, mx, my) +
