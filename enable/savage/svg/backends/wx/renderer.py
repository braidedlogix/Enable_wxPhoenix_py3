...
class Renderer(NullRenderer):
    ...
    def createAffineMatrix(a, b, c, d, x, y):
        return wx.GraphicsRenderer.GetDefaultRenderer().CreateMatrix(a, b, c,
                                                                     d, x, y)
    ...
    @staticmethod
    def createLinearGradientBrush(x1,
                                  y1,
                                  x2,
                                  y2,
                                  stops,
        ...
        if wx.VERSION > (2, 9):
            ...
            if len(stops) > 2:
                warnings.warn(
                    "wxPython 2.8 only supports 2 gradient stops, but %d were specified"
                    % len(stops))
