...
class TestEscaped(unittest.TestCase):
    ...
    def testEscapeDoesntMatchHexChars(self):
        ...
        for char in string.hexdigits:
            self.assertRaises(ParseException, identifier.escaped.parseString,
                              "\\" + char)


class TestHexUnicode(unittest.TestCase):
    parser = (identifier.hex_unicode.copy() + Regex(".+") + StringEnd()
              ).leaveWhitespace()
...
class TestEscape(unittest.TestCase):
    def testEscapeValues(self):
        ...
        self.assertEqual('\x81',
                         identifier.escape.parseString("\\" + chr(129))[0])
...
class TestNonAscii(unittest.TestCase):
    def testNoMatchInAsciiRange(self):
        for c in map(chr, list(range(128))):
            self.assertRaises(ParseException, identifier.nonascii.parseString,
                              c)
