...
zone_suffixes = [
    '_line_up_suffix', '', '_line_down_suffix', '', '_slider_suffix'
]

# Scroll information look-up table:
scroll_info = [('line_up', 1.0, 3), ('page_up', 1.0, 2),
               ('line_down', -1.0, 3), ('page_down', -1.0, 2)]
...
def valid_range(object, name, value):
    ...
    try:
        if (type(value) is tuple) and (len(value) == 4):
            ...
            return (float(low), float(high), float(page_size),
                    float(line_size))
    except:
        pass
    raise TraitError
...
class ScrollBar(Component):
    ...
    def _init_images(self):
        ...
        for name in [
                'aup', 'adown', 'aleft', 'aright', 'vtop', 'vbottom', 'vmid',
                'vpad', 'hleft', 'hright', 'hmid', 'hpad'
        ]:
    ...
    def _draw_vertical(self, gc):
        ...
        if page_size > 0.0:
            ...
            vmid_dy = vmid.height()
            gc.stretch_draw(sb_image['vpad' + suffix], x, s_y + vbottom_dy, dx,
            ...
            gc.draw_image(vmid, (x, round(s_y + vbottom_dy + (
                s_dy - vbottom_dy - vtop_dy - vmid_dy) / 2.0), dx, vmid_dy))
    ...
    def _draw_horizontal(self, gc):
        ...
        if page_size > 0.0:
            ...
            hmid_dx = hmid.width()
            gc.stretch_draw(sb_image['hpad' + suffix], s_x + hleft_dx, y,
            ...
            gc.draw_image(hmid, (round(s_x + hleft_dx + (
                s_dx - hleft_dx - hright_dx - hmid_dx) / 2.0), y, hmid_dx, dy))
