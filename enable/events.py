...
from traits.api import (Any, Bool, Float, HasTraits, Int, Event, List,
                        ReadOnly)
...
class KeyEvent(BasicEvent):
    ...
    def __repr__(self):
        s = (
            '%s(event_type=%r, character=%r, alt_down=%r, control_down=%r, shift_down=%r, handled=%r)'
...
class BlobEvent(BasicEvent):
    ...
    def __repr__(self):
        s = '%s(bid=%r, x=%r, y=%r, x0=%r, y0=%r, handled=%r)' % (
            self.__class__.__name__, self.bid, self.x, self.y, self.x0,
            self.y0, self.handled)
        return s
