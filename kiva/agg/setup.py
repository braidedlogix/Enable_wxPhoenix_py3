...
freetype2_dirs = [
    'autofit', 'base', 'bdf', 'cache', 'cff', 'cid', 'gxvalid', 'gzip', 'lzw',
    'otvalid', 'pcf', 'pfr', 'psaux', 'pshinter', 'psnames', 'raster', 'sfnt',
    'smooth', 'tools', 'truetype', 'type1', 'type42', 'winfonts', 'gzip'
]
...
def configuration(parent_package='', top_path=None):
    ...
    if ('NUMPY', None) in numerix_info.get('define_macros', []):
        dict_append(
            ...
            define_macros=[('PY_ARRAY_TYPES_PREFIX', 'NUMPY_CXX'),
                           ('OWN_DIMENSIONS', '0'), ('OWN_STRIDES', '0')])
    ...
    config.add_library(
        agg_lib, agg_sources, include_dirs=agg_include_dirs,
        depends=[agg_dir])
